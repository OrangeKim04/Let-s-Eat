...
export default function CreateRoom({ route, navigation }) {
  const { addRoom } = route.params; // 기존 방식은 경고를 발생시킵니다.
  ...
  const handleCreateRoom = () => {
    ...
    // addRoom을 navigation.setParams로 전달하는 방법으로 수정
    navigation.setParams({ addRoom: roomDetails });
        ...
        >
          <TouchableOpacity style={styles.modalOverlay} onPress={() => setMenuModalVisible(false)}>
            <View style={styles.modalContent}>
              {['한식', '일식', '중식'].map((category, index) => (
                <TouchableOpacity
                  key={index} // index 사용하여 고유한 key 값 제공
                  onPress={() => {
